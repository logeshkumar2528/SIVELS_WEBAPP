...
import { deleteProperty } from '../../../api/masters/propertyApi';
import { getErrorMessage } from '../../../utils/errorHelper';

export function PropertyDeleteConfirm({ isOpen, onClose, onSuccess, record }) {
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleDelete = async () => {
    if (!record || !record.propertyId) {
      console.error('Missing propertyId for deletion');
      return;
    }

    setIsSubmitting(true);
    try {
      const res = await deleteProperty(record.propertyId);
