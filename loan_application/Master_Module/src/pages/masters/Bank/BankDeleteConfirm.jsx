...
import { deleteBank } from '../../../api/masters/bankApi';
import { getErrorMessage } from '../../../utils/errorHelper';

export function BankDeleteConfirm({ isOpen, onClose, onSuccess, record }) {
  const [isDeleting, setIsDeleting] = useState(false);

  if (!record) return null;

  const handleDelete = async () => {
    if (!record || !record.bankId) {
      console.error('Missing bankId for deletion');
      return;
    }

    setIsDeleting(true);
    try {
      const response = await deleteBank(record.bankId);
