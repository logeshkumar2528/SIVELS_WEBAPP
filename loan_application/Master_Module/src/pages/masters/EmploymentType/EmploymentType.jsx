...
import { EmploymentTypeDeleteConfirm } from './EmploymentTypeDeleteConfirm';
...
export function EmploymentType() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(item => 
        item.employmentTypeName?.toLowerCase().includes(lowerSearch) ||
        item.employmentCode?.toLowerCase().includes(lowerSearch)
      );
    }
  ...
  const handleEdit = (row) => {
    if (!row || !row.employmentTypeId) {
      console.error('Invalid record or missing employmentTypeId:', row);
      return;
    }
      ...
      <div className="masters-page-toolbar">
        <div className="masters-page-search-area">
          <MasterSearch 
            ...
            placeholder="Search by Employment Code or Employment Type Name..."
          />
        </div>
