...
import { deleteDistrict } from '../../../api/masters/districtApi';
import { getErrorMessage } from '../../../utils/errorHelper';

export function DistrictDeleteConfirm({ isOpen, onClose, onSuccess, record }) {
  const [isDeleting, setIsDeleting] = useState(false);

  if (!record) return null;

  const handleDelete = async () => {
    if (!record || !record.districtId) {
      console.error('Missing districtId for deletion');
      return;
    }

    setIsDeleting(true);
    try {
      const response = await deleteDistrict(record.districtId);
