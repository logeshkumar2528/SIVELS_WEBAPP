...
import { deleteBankBranch } from '../../../api/masters/bankBranchApi';
import { getErrorMessage } from '../../../utils/errorHelper';

export function BankBranchDeleteConfirm({ isOpen, onClose, onSuccess, record }) {
  const [isDeleting, setIsDeleting] = useState(false);

  if (!record) return null;

  const handleDelete = async () => {
    if (!record || !record.bankBranchId) {
      console.error('Missing bankBranchId for deletion');
      return;
    }

    setIsDeleting(true);
    try {
      const response = await deleteBankBranch(record.bankBranchId);
